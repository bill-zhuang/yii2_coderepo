 ...
 */
if (jQuery)(function ($) {
    $.minicolors = {defaults: {animationSpeed: 50, animationEasing: 'swing', change: null, changeDelay: 0, control: 'hue', dataUris: true, defaultValue: '', hide: null, hideSpeed: 100, inline: false, letterCase: 'lowercase', opacity: false, position: 'bottom left', show: null, showSpeed: 100, theme: 'default'}};
    $.extend($.fn, {minicolors: function (method, data) {
        switch (method) {
            case'destroy':
                $(this).each(function () {
                    destroy($(this))
    ...
    function move(target, event, animate) {
        ...
        if (target.is('.minicolors-grid')) {
            picker.stop(true).animate({top: y + 'px', left: x + 'px'}, duration, settings.animationEasing, function () {
                updateFromControl(input, target)
            })
        } else {
            picker.stop(true).animate({top: y + 'px'}, duration, settings.animationEasing, function () {
                updateFromControl(input, target)
            })
        }
    }
    ...
    function updateFromControl(input, target) {
        function getCoords(picker, container) {
            ...
            return{x: left - container.offset().left + (picker.outerWidth() / 2), y: top - container.offset().top + (picker.outerHeight() / 2)}
        }
        ...
        if (target.is('.minicolors-grid, .minicolors-slider')) {
            switch (settings.control) {
                case'wheel':
                    ...
                    hex = hsb2hex({h: hue, s: saturation, b: brightness});
        ...
        swatch.find('SPAN').css({backgroundColor: hex, opacity: opacity});
    ...
    function updateFromInput(input, preserveInputValue) {
        ...
        switch (settings.control) {
            case'wheel':
                ...
                gridPicker.css({top: y + 'px', left: x + 'px'});
    ...
    function doChange(input, hex, opacity) {
        ...
        if (!lastChange || lastChange.hex !== hex || lastChange.opacity !== opacity) {
            input.data('minicolors-lastChange', {hex: hex, opacity: opacity});
            if (settings.change) {
